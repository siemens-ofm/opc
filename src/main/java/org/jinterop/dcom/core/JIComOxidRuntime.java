    ...
    private static class ServerPingTimerTask extends TimerTask
    {
        ...
        public void run ()
        {
            ...
            synchronized ( mutex2 )
            {
                ...
                //this is a bug, author originally use mapOfOIDVsComponents, i can't understand why, so i change the set to  mapOfSetIdVsListOfOIDs.
                final Iterator itr = mapOfSetIdVsListOfOIDs.keySet ().iterator ();
                ...
                while ( itr.hasNext () )
                {
                    ...
                    {
                        //remove all
                        JILocalCoClass component = (JILocalCoClass)mapOfSetIdVsListOfOIDs.get ( oid );
                        ...
                        logger.debug("ServerPingTimerTask interruptRemUnknownThreadGroup");
    ...
    static void destroySessionOIDs ( final int sessionId )
    {
        synchronized ( mutex2 )
        {
            ...
            for ( int i = 0; i < oids.size (); i++ )
            {
                ...
                if ( details != null )
                {
                    logger.debug("destroySessionOIDs interruptRemUnknownThreadGroup");
                	details.interruptRemUnknownThreadGroup ();
                }
                component = null;
                details = null;
